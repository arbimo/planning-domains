...
def generate_instance(num_hoists, num_steps, num_bars, dir_folder = None, durative_encoding = False) :

    if durative_encoding:
...
def main() :
    ...
    if auto_folder == None:
        generate_instance(int(args.num_hoists), int(args.num_positions), int(args.num_bars))  
    else:
        ...
        for i in range(1,int(args.num_positions)+1):
            for j in range(1,min(int(args.num_hoists),i-1)+1):
                for k in range(1,int(args.num_bars)+1):
                    generate_instance(j, i, k, auto_folder, args.durative == True)  
