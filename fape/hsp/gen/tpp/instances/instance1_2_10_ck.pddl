(define (control-knowledge ck1) (:domain mais) (:problem p1)
	...
	(t-exists (?prepare_barb8p0 - (prepare_bar b8 p0)) 
		...
		(t-exists (?finish_barb8p1 - (finish_bar b8 p1)) 
		(and (= (- ?loadh0b8p0 ?prepare_barb8p0) 60)(= (- ?finish_barb8p1 ?unloadh0b8p1) 60))))))
	(t-exists (?prepare_barb9p0 - (prepare_bar b9 p0)) 
		(t-exists (?loadh0b9p0 - (load h0 b9 p0)) 
		(t-exists (?unloadh0b9p1 - (unload h0 b9 p1)) 
		(t-exists (?finish_barb9p1 - (finish_bar b9 p1)) 
		(and (= (- ?loadh0b9p0 ?prepare_barb9p0) 60)(= (- ?finish_barb9p1 ?unloadh0b9p1) 60))))))
